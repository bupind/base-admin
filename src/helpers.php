...
if(!function_exists('admin_path')) {
    ...
    {
        return ucfirst(config('backend.directory')) . ($path ? DIRECTORY_SEPARATOR . $path : $path);
    }
}
if(!function_exists('admin_url')) {
    function admin_url($path = '', $parameters = [], $secure = null)
    {
        ...
        $secure = $secure ?: (config('backend.https') || config('backend.secure'));
...
if(!function_exists('admin_base_path')) {
    ...
    {
        $prefix = '/' . trim(config('backend.route.prefix'), '/');
...
if(!function_exists('admin_asset')) {
    ...
    {
        return (config('backend.https') || config('backend.secure')) ? secure_asset($path) : asset($path);
    }
}
...
if(!function_exists('admin_get_route')) {
    ...
    {
        return config('backend.route.prefix') . '.' . $name;
    }
}
