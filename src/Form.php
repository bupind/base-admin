...
use Base\Admin\Exception\Handler;
use Base\Admin\Form\Builder;
use Base\Admin\Form\Concerns\HandleCascadeFields;
use Base\Admin\Form\Concerns\HasFields;
use Base\Admin\Form\Concerns\HasFormAttributes;
use Base\Admin\Form\Concerns\HasHooks;
use Base\Admin\Form\Field;
use Base\Admin\Form\Layout\Layout;
use Base\Admin\Form\Row;
use Base\Admin\Form\Tab;
use Base\Admin\Grid\Tools\BatchEdit;
use Base\Admin\Traits\ShouldSnakeAttributes;
...
class Form implements Renderable
{
    ...
    public function destroy($id)
    {
        try {
            ...
            $response = [
                'status' => false,
                'message' => $exception->getMessage() ?: trans('backend.delete_failed'),
            ];
        }
    ...
    protected function redirectAfterSaving($resourcesPath, $key)
    {
        if (request('after-save') == 'continue_editing') {
            ...
            // return message
            return trans('backend.save_succeeded');
            exit;
        } elseif (strpos(request('_previous_'), 'ids')) {
            $url = (new BatchEdit(trans('backend.batch_edit')))->buildBatchUrl($resourcesPath);
        } else {
            $url = request(Builder::PREVIOUS_URL_KEY) ?: $resourcesPath;
        }

        admin_toastr(trans('backend.save_succeeded'));
