...
namespace Base\Admin\Form\Field\Traits;
...
trait HasMediaPicker
{
    ...
    {
        if (! class_exists("Base\Admin\Media\MediaManager")) {
            throw new \Exception(
                '[Media Manager extention not installed yet.<br> Install using: <b>composer require base-admin-ext/media-manager</b><br><br>'
            );
        }
        ...
        $this->addVariables([
            ...
            'trans' => [
                'choose' => admin_trans('backend.choose'),
                'cancal' => admin_trans('backend.cancel'),
                'submit' => admin_trans('backend.submit'),
            ],
        ]);

        $this->addPickBtn();

        return Admin::component('backend::components.mediapicker', $this->variables());
    }
}
