...
use Base\Admin\Auth\Permission as Checker;
...
class Permission
{
    ...
    {
        if (config('backend.check_route_permission') === false) {
            return $next($request);
        }
    ...
    /**
     * If the route of current request contains a middleware prefixed with 'backend.permission:',
    ...
    {
        // 下面这些路由不验证权限
        $excepts = array_merge(config('backend.auth.excepts', []), [
            'auth/login',
            'auth/logout',
            '_handle_action_',
            '_handle_form_',
