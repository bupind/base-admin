...
abstract class Extension
{
    ...
    /**
     * Get config set in config/backend.php.
     *
     * @param  string  $key
     * @param  null  $default
    ...
    public static function config($key = null, $default = null)
    {
        ...
        if (is_null($key)) {
            $key = sprintf('backend.extensions.%s', strtolower($name));
        } else {
            $key = sprintf('backend.extensions.%s.%s', strtolower($name), $key);
        }
    ...
    /**
     * Import menu item and permission to base-backend.
     */
    ...
    protected function getMenuValidationRules()
    {
        return [
            'title' => 'required',
            'path' => ['required', Rule::unique(Config::get('backend.database.menu_table'), 'uri')],
            'icon' => 'required',
            'children' => 'nullable|array',
        ];
    }
    ...
    protected function getPermissionValidationRules()
    {
        return [
            'name' => 'required',
            'slug' => ['required', Rule::unique(Config::get('backend.database.permissions_table'), 'slug')],
            'path' => 'required|array',
            'path.*' => 'string',
    ...
    /**
     * Create a item in base-admin left side menu.
     *
     * @param  string  $title
     * @param  string  $uri
     * @param  string  $icon
