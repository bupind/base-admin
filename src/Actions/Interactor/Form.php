...
namespace Base\Admin\Actions\Interactor;
...
use Base\Admin\Form as ModalForm;
...
class Form extends Interactor
{
    ...
    {
        if ($this->extendsFrom($this->action, 'Base\Admin\Actions\RowAction')) {
    ...
    protected function checkUploadFiel($field)
    {
        ...
        if ($this->extendsFrom($field, 'Base\Admin\Form\Field\File')) {
            $this->multipart = true;
        }
    }
    ...
    protected function resolveView($class)
    {
        ...
        if (! View::exists("backend::form.{$name}")) {
            $name = 'input';
        }

        return "backend::form.{$name}";
    }
    ...
    public function addModalHtml()
    {
        ...
        $modal = view('backend::actions.form.modal', $data)->render();
            ...
            document.querySelectorAll('{$this->action->selector($this->action->selectorPrefix)}').forEach(el=>{
                el.addEventListener('{$this->action->event}',function(){
                    ...
                    if (myModalEl.querySelector("[name='_key']").value == ""){
                        myModalEl.querySelector("[name='_key']").value = backend.grid.selected.join();
                    }

                    myModalEl.querySelector('form').addEventListener('submit',function(e){
                        e.preventDefault();
                        var form = this;
                        backend.form.submit(form,function(data){
