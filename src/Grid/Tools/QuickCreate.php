...
document.querySelector('.quick-create .create-form').addEventListener('submit',function (e) {
    ...
    backend.form.submit(form,function(data){
        if (data.status == 200) {
            backend.toastr.success("Saved",{positionClass:"toast-top-center"});
            backend.ajax.reload();
            return;
        }

        if (typeof data.validation !== 'undefined') {
            backend.toastr.warning(data.message, {positionClass:"toast-top-center"})
        }
    });
    return false;
});
JS;
    ...
    public function render($columnCount = 0)
    {
        ...
        return view('backend::grid.quick-create-form', $vars)->render();
    }
