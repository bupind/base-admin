    ...
    public function render()
    {
        if (! $this->enableEdit) {
            $this->actions = $this->actions->filter(function ($action, $key) {
                return get_class($action) != "Base\Admin\Grid\Tools\BatchEdit";
            });
        }

        if (! $this->enableDelete) {
            $this->actions = $this->actions->filter(function ($action, $key) {
                return get_class($action) != "Base\Admin\Grid\Tools\BatchDelete";
            });
        }

        $this->addActionScripts();

        return Admin::component('backend::grid.batch-actions', [
