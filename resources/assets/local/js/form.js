        ...
        Array.from(document.getElementsByTagName('form')).forEach((form) => {
            if (form.getAttribute('pjax-container') != null && !form.classList.contains('has-ajax-handler')) {
                form.addEventListener('submit', function (event) {
                    backend.form.submit(event.target);
        ...
        if (backend.form.validate(form)) {
            if (method === 'post' || method === 'put') {
                ...
                if (typeof result_function !== 'function') {
                    backend.ajax.setUrl(url);
                }
            }

            if (typeof result_function === 'function') {
                backend.ajax.request(url, obj, result_function);
            } else {
                backend.ajax.load(url, obj);
            }
        } else {
