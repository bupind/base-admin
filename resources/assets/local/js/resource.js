...
backend.resource = {
    delete: function (event, delete_link) {
        ...
        Swal.fire({
            title: __('delete_confirm'), type: "warning", showCancelButton: true, confirmButtonColor: "#dd6b55", confirmButtonText: __('confirm'), showLoaderOnConfirm: true, cancelButtonText: __('cancel'), preConfirm: function () {
                return new Promise(function (resolve) {
                    let url = resource_url;
                    let data = {_method: 'delete'};
                    backend.ajax.post(url, data, function (data) {
                        resolve(data);
                        if (navigate_url) {
                            backend.ajax.navigate(navigate_url);
                        } else {
                            backend.ajax.reload();
                        }
                    });
        ...
        }).then(backend.resource.default_swal_response);
        ...
        backend.ajax.navigate(edit_url);
        ...
        backend.ajax.reload();
        backend.resource.default_swal_response(result);
    }
}
