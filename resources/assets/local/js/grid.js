...
/*-------------------------------------------------*/
    ...
    backend.grid = {
        ...
        check_status : function(){
            ...
            document.querySelectorAll(".grid-row-checkbox").forEach( box => {
                ...
                if (!box.checked){
                    ...
                    if (!backend.grid.selected.includes(box.dataset.id)){
                        backend.grid.selected.push(box.dataset.id);
                        num_selected ++;
                    }
                }
            })
        ...
        export_selected_row : function(event){
            ...
            let rows = backend.grid.selected.join();
            if (rows == ""){
                Swal.fire({
                    icon: 'info',
        ...
        columns : {
            ...
            submit : function(){
                ...
                backend.ajax.navigate(url.toString());
        ...
        hotkeys : function() {
            ...
            document.removeEventListener("keydown", backend.grid.hotkeys_handle,false);
            document.addEventListener("keydown", backend.grid.hotkeys_handle,false);
        },
        ...
        hotkeys_handle : function (e){
            ...
            switch(e.which) {
                ...
                case 82: // `r` for reload
                    backend.ajax.reload();
                    backend.toastr.success(__('refresh_succeeded'), {positionClass:"toast-top-center"});
                    break;
                ...
                case 46: // `delete` batch delete
                    if (backend.grid.selected.length){
