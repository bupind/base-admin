        ...
        <div class="card card-primary">
            <div class="card-header with-border">
                <button type="button" class="btn btn-primary btn-sm log-refresh">
                    <i class="icon-refresh"></i> {{ trans('backend.refresh') }}</button>
                <button type="button" class="btn btn-light btn-sm log-live">
                    <i id="live-indicator" class="icon-play"></i></button>
                <div class="float-end">
                    <div class="btn-group">
                        @if ($prevUrl)
                            ...
                            @if (!empty($log['trace']))
                                <tr>
                                    <td colspan="5" class="p-0 border-0">
                                        <div class="collapse" id="trace-{{$index}}" style="white-space: pre-wrap;background: #333;color: #fff; padding: 10px;">{{ $log['trace'] }}</div>
                                    </td>
                                </tr>
                            @endif
                        @endforeach
                ...
                <ul class="nav nav-pills flex-column">
                    @foreach($logFiles as $logFile)
                        <li @if($logFile == $fileName)class="active"@endif>
                            <a href="{{ route('log-viewer-file', ['file' => str_replace($bypass_protected_urls_find,$bypass_protected_urls_replace,$logFile)]) }}"><i class="icon-{{ ($logFile == $fileName) ? 'folder-open' : 'folder' }}"></i>{{ $logFile }}
                            </a>
                        </li>
                    @endforeach
                </ul>
...
<script data-exec-on-popstate>
    ...
    (function () {
        ...
        function fetch() {
            var url = '{{ $tailPath }}';
            var data = {offset: pos}
            backend.ajax.get(url, data, function (response) {
