...
<header class="custom-navbar navbar navbar-light bg-white p-0 align-items-stretch">
    <a class="navbar-brand menu-width container-md bg-semi-dark text-center" href="{{ admin_url('/') }}">
        <span class="short">{!! config('backend.logo-mini', config('backend.name')) !!}</span><span class="long">{!! config('backend.logo', config('backend.name')) !!}</span>
    </a>
                ...
                <ul class="dropdown-menu dropdown-menu-end user-menu" aria-labelledby="user-menu-link">
                    ...
                    <li class="user-footer p-2 clearfix">
                        <div class="float-start">
                            <a href="{{ admin_url('auth/setting') }}" class="btn btn-secondary">{{ __('backend.setting') }}</a>
                        </div>
                        <div class="float-end">
                            <a href="{{ admin_url('auth/logout') }}" class="btn no-ajax btn-secondary">{{ __('backend.logout') }}</a>
                        </div>
                    </li>
                </ul>
            </li>
